...
实现语言：Python 3.11+，纯 CLI，无 GUI。本文档是实现规约：按它复现应得到行为等价的工具。
...
约束：
- **纯计算层**不做 IO、不输出、不读取环境或文件；只依赖显式输入与领域常量，所有可配置参数通过一个 dataclass 注入
...
开发与验证使用 uv：`uv sync --extra dev`，入口命令为 `uv run ppt`。如需传统安装，也可使用 `pip install -e .`。
