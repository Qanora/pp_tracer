...
| 参数 | 默认值 | 含义 |
|---|---|---|
| `rebalance.target` | 0.25 | 等权目标 |
| `rebalance.tolerance` | 0.005 | 缺口识别阈值（占总资产比例） |
| `rebalance.upper_limit` | 0.35 | 固定走廊上限（fallback，无价格历史时用） |
| `rebalance.lower_limit` | 0.15 | 固定走廊下限（fallback） |
...
| `advanced.trend_short_window` | 10 | 短期均线天数 S |
| `advanced.trend_long_window` | 20 | 长期均线天数 L |
